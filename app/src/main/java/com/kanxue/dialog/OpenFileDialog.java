...
import com.kanxue.desencrypt.R;
...
public class OpenFileDialog {
    static final public String _dir ="yyf";
    static final public String TAG ="qqq";
    static final public String sRoot = "/sdcard/"+_dir;
...
//过滤文件的字符串
        private String getSuffix(String filename) {
            int dix = filename.lastIndexOf("test");
            Log.d(TAG, "filename: "+filename+dix);
            if (dix < 0) {
                return "";
            } else {
   ...
   //非sdcard/yyf路径 触发
            if (!this.path.equals(sRoot)) {
                ...
                list.add(map);
                Log.w(TAG, "!this.path.equals(sRoot): ");
...
//                    Log.w(TAG, " getSuffix(file.getName()).toLowerCase: "+sf);
